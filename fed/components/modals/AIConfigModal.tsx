    ...
    <ModalOverlay className="animate-fade-in" role="dialog" aria-modal="true">
      <div className="modal-sheet max-w-md md:animate-slide-up">
        ...
        <div className="p-5 md:p-7 border-t border-slate-50 flex flex-col-reverse sm:flex-row sm:justify-end gap-2 sm:space-x-3 bg-slate-50/20 flex-shrink-0">
          <button 
            onClick={onClose} 
            className="px-5 py-2.5 text-[12px] font-bold text-slate-400 hover:text-slate-600 transition-colors duration-200 uppercase tracking-widest cursor-pointer min-h-[44px]"
          >
            取消
          </button>
          <button 
            ...
            className="px-8 py-2.5 bg-brand-600 hover:bg-brand-700 disabled:bg-slate-300 text-white font-bold rounded-xl shadow-sm active:scale-95 text-[12px] uppercase tracking-widest cursor-pointer transition-colors duration-200 min-h-[44px]"
          >
            {isSaving ? '保存中...' : '保存配置'}
          </button>
        </div>
